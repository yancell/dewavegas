<?php
	...
	$ch5 = curl_init();
	curl_setopt($ch5, CURLOPT_URL, "https://dewafortune.xyz/auth/select_game_v2.php");
	...
	$ch6 = curl_init();
	curl_setopt($ch6, CURLOPT_URL, "https://dewafortune.xyz/var_api.php");
		...
		foreach ($arr as $k => $v){
			if (!empty($v)){
				...
				if (in_array($var[$k], $target)){
					$ch7 = curl_init();
					curl_setopt($ch7, CURLOPT_URL, "https://dewafortune.xyz/bq2.php");
					curl_setopt($ch7, CURLOPT_COOKIEFILE, "cookie1".$user.".txt");
					curl_setopt($ch7, CURLOPT_RETURNTRANSFER, 1);
					curl_setopt($ch7, CURLOPT_CUSTOMREQUEST, "POST");
					curl_setopt($ch7, CURLOPT_POST, 1);
					curl_setopt($ch7, CURLOPT_POSTFIELDS, 'resres='.$res[$k].'&res='.$res[$k]);
					curl_exec($ch7);
					curl_close($ch7);
				}
				break;
